...
from moco_dataloader import HardMixMatchImageLoader
...
def _moco_infer(model, root_path, test_loader=None):
    ...
    if test_loader is None:
        test_loader = torch.utils.data.DataLoader(
            HardMixMatchImageLoader(root_path, 'test',
...
def bind_nsml_ema(model):
    """
    bind function for a set of models : model, ema_model
    :param model: Tuple of (model, ema_model)
    :return:
    """
    def save(dir_name, *args, **kwargs):
        student, teacher = model[0], model[1]
        os.makedirs(dir_name, exist_ok=True)
        state = student.state_dict()
        torch.save(state, os.path.join(dir_name, 'student.pt'))
        state = teacher.ema.state_dict()
        torch.save(state, os.path.join(dir_name, 'teacher.pt'))
...
parser.add_argument('--lr', type=float, default=.03, metavar='LR', help='learning rate (default: 5e-5)')
...
parser.add_argument('--use_sup_pretrained', action='store_true', default=False,help = 'use supervisely pretrained')
parser.add_argument('--use_moco', action='store_true',default=False, help = 'use moco')

parser.add_argument('--release', action='store_true',default=False,  help = 'unfreeze more layers to update')
parser.add_argument('--finetuning', action='store_true',default=False, help = 'use finetuning with freezing the rest layers')
parser.add_argument('--retrain', action='store_true', default=False, help = 'retrain')
...
# my_idea
parser.add_argument('--four_imgs', action='store_true', default=False, help='use fixmatch-ish')
parser.add_argument('--sgd', action='store_true', default=False, help='use sgd lr.03 + cosine')
parser.add_argument('--thrh', type=float, default=.5, help='sharpening or pseudo labelling thrh')
...
def main():
    ...
    if IS_ON_NSML:
        if opts.use_ema :
            ...
            print("ema_model is set")
            bind_nsml_ema((model, ema_model))
        else :
            bind_nsml(model)
        if opts.pause:
    ...
    if opts.mode == 'train':
        ...
        train_loader = torch.utils.data.DataLoader(
            HardMixMatchImageLoader(DATASET_PATH, 'train', train_ids,
                              transform=transforms.Compose([
        ...
        if opts.four_imgs:
            unlabel_loader = torch.utils.data.DataLoader(
                HardMixMatchImageLoader(DATASET_PATH, 'unlabel', unl_ids,
                                  transform=transforms.Compose([
                                      transforms.Resize([opts.imsize, opts.imsize]), #opts.imResize
                                      # transforms.RandomResizedCrop(opts.imsize),
                                      transforms.RandomHorizontalFlip(),
                ...
                batch_size=opts.batchsize, shuffle=True, num_workers=4, pin_memory=True, drop_last=True)
            print('unlabel_loader with transform_strong is ready')
        ...
        validation_loader = torch.utils.data.DataLoader(
            HardMixMatchImageLoader(DATASET_PATH, 'val', val_ids,
                              transform=transforms.Compose([
                                  transforms.Resize(opts.imResize),
                                  transforms.CenterCrop(opts.imsize),
                                  transforms.ToTensor(),
        ...
        # Set optimizer
        optimizer = optim.SGD(model.parameters(), lr=opts.lr, momentum=.9, nesterov=True) if opts.sgd else optim.Adam(model.parameters(), lr=opts.lr)
        print("{} is set".format("sgd" if opts.sgd else "adam"))
        ...
        for epoch in range(opts.start_epoch, opts.epochs + 1):
            ...
            if opts.four_imgs:
                if opts.use_ema :
                    loss, train_acc_top1, train_acc_top5, train_i = train(opts, train_loader, unlabel_loader, model,
                                                                          train_criterion, optimizer, epoch, use_gpu,
                                                                          train_i, ema_model=ema_model,scheduler=scheduler)
                else :
                    loss, train_acc_top1, train_acc_top5, train_i = train(opts, train_loader, unlabel_loader, model,
                                                                          train_criterion, optimizer, epoch, use_gpu,
                                                                          train_i, scheduler=scheduler)
            else :
                loss, train_acc_top1, train_acc_top5, train_i = train(opts, train_loader, unlabel_loader, model,
                                                                      train_criterion, optimizer, epoch, use_gpu,
                                                                      train_i, )
...
def train(opts, train_loader, unlabel_loader, model, criterion, optimizer, epoch, use_gpu, train_i, ema_model=None, scheduler=None):
    losses = AverageMeter()
    ...
    for batch_idx in range(len(train_loader)):
        ...
        if opts.four_imgs :
            inputs_u1_str, inputs_u2_str = Variable(inputs_u1_str), Variable(inputs_u2_str)
            if use_gpu:
                inputs_u1_str, inputs_u2_str = inputs_u1_str.cuda(), inputs_u2_str.cuda()
            # print("strongly transformed imgs are made")

        with torch.no_grad():
            # compute guessed labels of unlabel samples : weakly sampled
            ...
            if opts.four_imgs and epoch > 30 : # sharpening or pseudo-label
                ...
                if pseudo_label_idx.any(): # 하나라도 true면 실행
                    cnt_ge_thrh += torch.sum(pseudo_label_idx).item()
                    _, indice = torch.max(pt[pseudo_label_idx], dim=1) # indice : (len(pseudo_label_idx), )
                    targets_u[pseudo_label_idx] = torch.zeros(targets_u[pseudo_label_idx].size()).cuda().scatter_(1, indice.view(-1, 1), 1)
                # print("sharpening and pseudo-label are done")
            else : # sharpening
                pt = pred_u_all ** (1 / opts.T)
                targets_u = pt / pt.sum(dim=1, keepdim=True)
                # print("only sharpening is done")
        ...
        if batch_idx % opts.log_interval == 0:
            print('Train Epoch:{} [{}/{}] Loss:{:.4f}({:.4f}) Top-1:{:.2f}%({:.2f}%) Top-5:{:.2f}%({:.2f}%) cnt_GE_thrh:{:.2f}({:.4f})'.format(
                epoch, batch_idx * inputs_x.size(0), len(train_loader.dataset), losses.val, losses.avg, acc_top1.val,
                acc_top1.avg, acc_top5.val, acc_top5.avg, cnt_ge_thrh, cnt_ge_thrh if batch_idx == 0 else cnt_ge_thrh/(batch_idx * inputs_x.size(0))))
