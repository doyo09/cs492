...
from utils import bind_nsml, load_model

def get_model_sup(opts):
    model = Resnet50(base_encoder=models.__dict__["resnet50"])
    load_model(model, opts)

    if opts.finetuning:
        for param in model.parameters():
            param.requires_grad = False
        assert len([param.size for param in model.parameters() if param.requires_grad]) == 0
    return model

def get_model_moco(opts):
    model = MoCoV2(base_encoder=models.__dict__["resnet50"])
    load_model(model, opts)
    ...
    model = MoCoClassifier(model, ClassifierBlock(), use_bn=True, pretrained=True, init=True)
    return model
...
class MoCoV2(nn.Module):
    def __init__(self, base_encoder, similarity_dim=128, q_size=200*179, momentum=0.999, temperature=0.07, ):
        ...
        self.q_enc = base_encoder(pretrained=True)
        self.k_enc = base_encoder(pretrained=True)
...
class Resnet50(nn.Module):
    ...
    def __init__(self, base_encoder, num_classes = 265):
        ...
        self.model_ft = base_encoder(pretrained=True)
